...
class NotificationFragment : androidx.fragment.app.Fragment() {
    ...
    private fun saveNotif(dataSnapshot: DataSnapshot,tripID:String,origin:String,dest:String,date:String){
        for (user in listOfLiteUser){
            ...
            if (tripID == notifications.last().tripID && user == listOfLiteUser.last()){
                ...
                notif_list_res_view.adapter = this.context?.let { NotificationAdapter(it,extendedNotification) }
                setPb(0)
            }
        }
    }
   private fun saveLiteUser(dataSnapshot: DataSnapshot,timestamp: String):LiteUser{
       ...
       if (fullName == null){
        ...
        val lU =  LiteUser(userID!!,fullName.toString(),gender!!,birthday!!,ratings.toDouble(),peopleDriven.toInt())
    ...
    private fun getTripData(dataSnapshot: DataSnapshot){
        ...
        var origin:String? = dataSnapshot.child("origin").value as String
        var dest = dataSnapshot.child("destination").value as String
        var date = dataSnapshot.child("date").value as String
        val tripID = dataSnapshot.key

        for (notification in notifications){
            Log.i("SucCddESs", dataSnapshot.childrenCount.toString()!!)
            if (notification.tripID == tripID){
                Log.i("if_loop", dataSnapshot.childrenCount.toString()!!)
